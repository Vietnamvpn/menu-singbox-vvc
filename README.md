...
├── core/
│   └── api_server.go           # Daemon xử lý API, traffic, user và tính toán dung lượng
├── modules/
│   ├── system.sh               # Quản lý service và firewall
│   ├── nodes.sh                # Quản lý thông tin và cấu hình Node
│   ├── entry-node.sh           # Quản lý cổng và domain entry theo tag
│   ├── users.sh                # Thêm, sửa, xóa và khóa user
│   ├── outbound.sh             # Quản lý Node relay
│   ├── routing.sh              # Quản lý quy tắc đầu ra
│   ├── ssl.sh                  # Xin chứng chỉ Cloudflare
│   ├── api-web.sh              # Gọi API web trung tâm
│   └── utils.sh                # Các tiện ích dùng chung
├── data/
│   ├── nodes.json              # Thông tin Node
│   ├── users.json              # Thông tin user
│   ├── outbound.json           # Thông tin Node relay
│   ├── routing.json            # Quy tắc đầu ra
│   ├── domain.json             # Thông tin domain theo tag node
│   └── entry-node.json         # Thông tin cổng và domain entry
├── certs/                      # Chứng chỉ SSL
├── logs/                       # Log của hệ thống
└── templates/
    ├── config.base.json
    ├── sing-box.service
    ├── manager.service
